...
3. `master` を pull し、コミット済みの生成パーサが最新であることを確かめてから gem をビルド・公開する

   ```bash
   bundle exec rake gen_force && git diff --exit-code -- lib/oracle-sql-parser/grammar
   bundle exec rake release   # v<version> タグの push と rubygems.org への公開
   ```

   生成パーサ（`grammar/**/*.rb`）は git 管理されていて `spec.files` は `git ls-files` から作られるので、差分が出たら生成漏れがコミットされていない。公開後は `gem contents oracle-sql-parser` で `grammar/*.rb` が含まれることを確認する
